...
/*
	...
	리턴하는 작업을 수행
		=> 미리 요청 path별로 실행할 실제 클래스를 등록한 설정차일을 보고 객체를 찾아서
		생성 mycommand.properties 파일에 등록된 요청 path를 보고 실행할 객체를 찾는다.
		xml 파일이나 properties 파일을 파싱하는 코드를 정의
*/
public class RequestMapping {
	//요청path가 key, 실제실행할 객체 value
	...
	public Action mapping(String path) {
		//매개변수로 전달된 path로 등록된 Actio객체를 actionMap에서 찾아서 리턴
